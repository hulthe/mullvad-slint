...
impl Map {
    pub fn new(device: &wgpu::Device, queue: &wgpu::Queue, size: slint::PhysicalSize) -> Self {
        ...
        // Load geometry data
        let land_points_bytes = include_bytes_aligned!(4, "../geo/land_positions.gl");
        let land_points: &[[f32; 3]] = bytemuck::cast_slice(land_points_bytes.as_slice());

        let land_indices_bytes = include_bytes_aligned!(4, "../geo/land_triangle_indices.gl");
        let land_indices: &[u32] = bytemuck::cast_slice(land_indices_bytes);

        let contour_indices_bytes = include_bytes_aligned!(4, "../geo/land_contour_indices.gl");
